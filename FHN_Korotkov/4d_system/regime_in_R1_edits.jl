...
u0_start = SVector{4}(u0_start[1:4]);
...
ds = CoupledODEs(FHN2_4d, last_point[index_p1,index_p2,1:4], parameters,
...
function d(p1, p2)
    ...
    dist = sqrt( (p1x1-p2x1)^2 + (p1y1-p2y1)^2 + (p1x2-p2x2)^2 + (p1y2-p2y2)^2)
    return dist
end
