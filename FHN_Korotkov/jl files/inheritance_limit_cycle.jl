...
filenameu0s = "u0s_k1_length_1000.jld2"
filenameLSEs = "LSEs_k1_length_1000.jld2"

inheritance_limit_cycle(index_param, range_param, tspan, time_calculate_LLE)

#=parameters = FHN2_try3_params()
parameters[7] = rangek1[1]

prob = ODEProblem(FHN2_try3, point, tspan, parameters)
sol = solve(prob, RK4(), adaptive = false, dt = 0.001, maxiters = 5e6)

tstart = round(Int, length(sol) / 2); tend = length(sol)
