...
using StaticArrays, DifferentialEquations, Statistics, CairoMakie, GLMakie, JLD2
...
function get_amplitude_all_spikes(array_spikes_thresholds, array_spikes_max)
    amplitudes = Float64[]
...
amplitudes_all_spikes = get_amplitude_all_spikes(array_spikes_thresholds, array_spikes_max)


f = Figure(size = (height_window, width_window))
ax = Axis(f[1, 1])
lines!(ax, amplitudes_all_spikes, linewidth = 1.0, color = :black)
display(GLMakie.Screen(), f)

println("len local_max: $(length(array_spikes_max))")
println("len all amplitude: $(length(amplitudes_all_spikes))")=#