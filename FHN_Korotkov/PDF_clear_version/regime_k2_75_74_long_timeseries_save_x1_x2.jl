...
function save_data(iteration, datax1, datax2, path_to_save)

    file_name_x1 = "$(iteration)_sol_x1.jld2"
    file_name_x2 = "$(iteration)_sol_x2.jld2"

    full_path_x1 = path_to_save * file_name_x1
    full_path_x2 = path_to_save * file_name_x2

    jldsave(full_path_x1; datax1)
    jldsave(full_path_x2; datax2)
end
...
function first_iteration(u0, parameters, tspan, integrator_setting, path_to_save)
    ...
    save_data(1, data_x1, data_x2, path_to_save)
    return last_point
end
...
function calculate_timeseris(u0_start, parameters, integrator_setting,
    ...
    for iteration in range(2, count_iteration)
        ...
        save_data(iteration, data_x1, data_x2, path_to_save)
