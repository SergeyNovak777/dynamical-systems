...
function calculate_LLE(ds, time_calculate_LLE)
    LLE = lyapunov(ds, time_calculate_LLE)
    return LLE
end
...
function save_in_matrix(index_cycle_p1, index_cycle_p2, dim,
    ...
    λs[index_cycle_p1, index_cycle_p2] = LLE
...
function save_in_files(namefile_LLE, namefile_u0s, dim)
    jldsave(namefile_LLE; λs)
